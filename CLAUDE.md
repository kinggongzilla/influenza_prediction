...
- **make_uk_subregions.py**: One-shot generator for `frontend/public/data/uk_subregions.json` (OSM admin-4 boundaries for England/Scotland/N. Ireland via Overpass, needs a User-Agent header). Outer-way nodes are rasterized to a 0.1° grid, flood-filled into solid land, and the contour is extracted with Jarnik's pixel-boundary tracing → always a simple closed ring (self-intersecting rings are the bug to avoid: d3-geo renders them as the *complement* of the polygon, i.e. the whole globe). Simplified with Douglas-Peucker, and the generator validates its own output with the same d3-geo the frontend uses (small spherical area, single path subpath, bounded extent) — it refuses to write a file that isn't a safe overlay. Re-run only if the file is lost.
